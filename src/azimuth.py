...
def get_args_parser():
    ...
    parser.add_argument('--sc_height', default = 251.5e3)
    parser.add_argument('--sc_vel', default = 3.397e3)
    parser.add_argument('--prf', default = 135)
    # Additional
    parser.add_argument('--start', default = 700, type = int)
    parser.add_argument('--stop', default = 800, type = int)
    parser.add_argument('--aperture', default = 8, type = float, help='Aperture in seconds')
    return parser
...
def focus(R, args):
    ...
    shifts = np.power(v * t, 2) / (2 * r)
    shifts_in_pixels = -shifts / dx
    N = R.shape[0]
    freq = np.fft.fftshift(np.fft.fftfreq(N, d=dx))
    R_fft_shifted = np.zeros_like(R_fft, dtype=np.complex128)
    for i in range(R.shape[1]):
        phase_shift = np.exp(-2j * np.pi * freq * shifts_in_pixels[i])
        R_fft_shifted[:, i] = R_fft[:, i] * phase_shift
