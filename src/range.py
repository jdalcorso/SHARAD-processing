...
def get_args_parser():
    ...
    parser.add_argument('--output_path', default = './data/raw_data.npy')
...
def main(args):
    ...
    T = si * samples
    t_signal = np.linspace(0,T - 1/sf, samples)
    
    # Range compression
    R_complex = R * np.exp(2 * np.pi * (sf-cf) *1j * t_signal)[:,np.newaxis]    # Complex demodulation
